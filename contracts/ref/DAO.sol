...
contract DAOInterface {
    ...
    function newProposal(
        ...
        bool _newCurator
    ) payable returns (uint256 _proposalID);
    ...
    function numberOfProposals() constant returns (uint256 _numberOfProposals);
...
contract DAO is DAOInterface {
    ...
    ) payable onlyTokenholders returns (uint256 _proposalID) {
        if (
    ...
    function verifyPreSupport(uint256 _proposalID) {
        Proposal p = proposals[_proposalID];
        if (now < p.votingDeadline - preSupportTime) {
            if (p.yea > p.nay) {
                p.preSupport = true;
            } else p.preSupport = false;
        }
    }
        ...
        returns (bool _success)
    {
        ...
        uint256 quorum = p.yea;

        // require max quorum for calling newContract()
        if (
            _transactionData.length >= 4 &&
            _transactionData[0] == 0x68 &&
            _transactionData[1] == 0x37 &&
            _transactionData[2] == 0xff &&
...
*/
    ...
    function actualBalance() constant returns (uint256 _actualBalance) {
        return this.balance - sumOfProposalDeposits;
    }

    function minQuorum(uint256 _value)
        internal
        constant
    ...
    function halveMinQuorum() returns (bool _success) {
        ...
        // between the calls
        if (
            (lastTimeMinQuorumMet < (now - quorumHalvingPeriod) ||
                msg.sender == curator) &&
